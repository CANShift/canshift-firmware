...
void show() {
    ...
    const int16_t marginX = LayoutScale::x(kMarginPx);

    lv_obj_t *header = BrandMark::create(scr, false);
    if (header)
        lv_obj_align(header, LV_ALIGN_TOP_LEFT, marginX, LayoutScale::y(22));

    char verBuf[24];
    snprintf(verBuf, sizeof(verBuf), "fw " APP_VERSION_STR);
    lv_obj_t *ver = makeMonoLabel(scr, verBuf, kDimRgb, 10);
    if (ver)
        lv_obj_align(ver, LV_ALIGN_TOP_RIGHT, -marginX, LayoutScale::y(34));

    lv_obj_t *title = makeMonoLabel(scr, "READY TO CONFIGURE", kInkRgb, 16);
    if (title)
        lv_obj_align(title, LV_ALIGN_TOP_LEFT, marginX, LayoutScale::y(92));

    lv_obj_t *intro = makeMonoLabel(scr, "No dashboard config on this device.", kDimRgb, 10);
    if (intro)
        lv_obj_align(intro, LV_ALIGN_TOP_LEFT, marginX, LayoutScale::y(120));
    ...
    if (how)
        lv_obj_align(how, LV_ALIGN_TOP_LEFT, marginX, LayoutScale::y(174));

    const int16_t waitSquarePx = LayoutScale::square(kWaitSquarePx);
    lv_obj_t *waitSquare = WidgetHelpers::makeSquareBadge(scr, waitSquarePx, kAccentRgb);
    if (waitSquare)
        lv_obj_align(waitSquare, LV_ALIGN_BOTTOM_LEFT, marginX, -LayoutScale::y(19));

    lv_obj_t *waiting = makeMonoLabel(scr, "WAITING FOR USB", kAccentRgb, 10);
    if (waiting)
        lv_obj_align(waiting, LV_ALIGN_BOTTOM_LEFT, marginX + waitSquarePx + LayoutScale::x(6),
                     -LayoutScale::y(17));

...
//
// On a freshly-flashed device with an empty SD card, the firmware writes its
// embedded default dashboard / signals / theme JSON to the canonical SD paths
// so the dashboard renders out of the box (issue #173).
//
// The default JSON blobs are linked into the firmware via PlatformIO's
// `board_build.embed_files` (see platformio.ini) — single source of truth
// shared with the manual `pio run -t flash_sd` workflow.
...
struct ProvisionResult {
    ...
    uint8_t failed;  // files that needed write but the SD write failed
};

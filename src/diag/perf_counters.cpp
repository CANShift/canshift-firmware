...
const char *metricName(PerfCounters::Metric m) {
    switch (m) {
        case PerfCounters::MUTEX_WAIT:    return "lock";
        case PerfCounters::WIDGETS:       return "widgets";
        case PerfCounters::TOPBAR:        return "topbar";
        case PerfCounters::LV_HANDLER:    return "lvgl";
        case PerfCounters::FRAME_TOTAL:   return "frame";
        case PerfCounters::PAGE_XSITION:  return "xsition";
        case PerfCounters::TOUCH_LATENCY: return "touch";
        default:                          return "?";
    }
}
...
bool consumeTouchPressTs(uint32_t *outUs) {
    if (!s_touchPressArmed) return false;
...
void tick() {
    ...
    // Build a single line: PERF lock=mean/p95/max widgets=… frame=… miss=N
    ...
    for (uint8_t i = 0; i < METRIC_COUNT; ++i) {
        ...
        const int n = snprintf(buf + written, sizeof(buf) - written,
                               "%s=%u/%u/%u ", metricName(static_cast<Metric>(i)),
                               static_cast<unsigned>(mean), static_cast<unsigned>(p95),
                               static_cast<unsigned>(w.maxUs));
        if (n < 0 || static_cast<size_t>(n) >= sizeof(buf) - written) break;
        written += n;
        w.clear();
    }
    if (written == 0) {
    ...
    LOG_INFO("PERF", "%smiss=%u", buf, static_cast<unsigned>(s_frameMissCount));
    s_frameMissCount = 0;
}
...
ScopedTimer::~ScopedTimer() {
    ...
    if (delta < 0 || delta > 1000000) return; // sanity cap (1 s) — discard runaway samples

...
void ThemeManager::apply() {
    ...
    lv_disp_t *disp = lv_disp_get_default();
    lv_theme_t *th =
        lv_theme_default_init(disp, lv_color_hex(theme.primary.rgb), lv_color_hex(theme.accent.rgb),
                              true, // Dark mode — automotive dash is always dark
                              LV_FONT_DEFAULT);
    lv_disp_set_theme(disp, th);

    // Apply background to default screen
    lv_obj_set_style_bg_color(lv_scr_act(), lv_color_hex(theme.background.rgb), LV_PART_MAIN);

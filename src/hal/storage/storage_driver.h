 ...
 * file in a contiguous heap buffer first. Avoids the contiguous malloc that
 * the legacy file-read path used (issue #576). Returns a non-empty
 ...
 * chunks, avoiding any contiguous file-sized allocation. Returns the number
 * of bytes actually written, or 0 on open failure.
 *

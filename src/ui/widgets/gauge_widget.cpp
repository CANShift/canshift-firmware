...
namespace {
    ...
    lv_color_t getArcColor(float value, const CfgWidget& cfg) {
        if (value >= cfg.gauge.dangerLevel)  return lv_color_hex(cfg.style.criticalColor.rgb);
        if (value >= cfg.gauge.warningLevel) return lv_color_hex(cfg.style.warningColor.rgb);
...
lv_obj_t* GaugeWidget::create(lv_obj_t* parent, const CfgWidget& cfg, int16_t yOffset) {
    ...
    lv_obj_set_style_arc_color(arc, lv_color_hex(cfg.style.secondaryColor.rgb),
                                LV_PART_MAIN);
    lv_obj_set_style_arc_width(arc, 8, LV_PART_MAIN);

    // Style — value arc
    lv_obj_set_style_arc_color(arc, lv_color_hex(cfg.style.primaryColor.rgb),
                                LV_PART_INDICATOR);
    ...
    if (cfg.layout.h >= 100) font = &lv_font_montserrat_24;
    if (cfg.layout.h >= 130) font = &lv_font_montserrat_32;
...
void GaugeWidget::update(lv_obj_t* obj, float value, bool valid, const CfgWidget& cfg) {
    ...
    if (!valid) {
        ...
        lv_obj_set_style_arc_color(tag->arc,

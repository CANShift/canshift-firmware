...
namespace {
    ...
    void nvsLoad() {
        ...
        if (s_brightness < 10 || s_brightness > 100) s_brightness = DEFAULT_BRIGHTNESS;
        if (s_contrast   > 100)                       s_contrast   = DEFAULT_CONTRAST;
    }
    ...
    void updateSleepButtons() {
        for (uint8_t i = 0; i < SLEEP_OPTION_COUNT; ++i) {
            if (!s_sleepBtns[i]) continue;
            bool active = (SLEEP_OPTIONS[i] == s_sleepTimeoutS);
            lv_obj_set_style_bg_color(s_sleepBtns[i],
                lv_color_hex(active ? CLR_BTN_ACT : CLR_BTN_BG), LV_PART_MAIN);
            lv_obj_set_style_border_color(s_sleepBtns[i],
                lv_color_hex(active ? CLR_ACCENT : CLR_BTN_BDR), LV_PART_MAIN);
            lv_obj_t* lbl = lv_obj_get_child(s_sleepBtns[i], 0);
            if (lbl) {
                lv_obj_set_style_text_color(lbl,
                    lv_color_hex(active ? CLR_ACCENT : CLR_MUTED), 0);
            }
        }
    }
    ...
    void updateRotButtons() {
        for (uint8_t i = 0; i < ROT_OPTION_COUNT; ++i) {
            if (!s_rotBtns[i]) continue;
            bool active = (ROT_OPTIONS[i] == s_rotation);
            lv_obj_set_style_bg_color(s_rotBtns[i],
                lv_color_hex(active ? CLR_BTN_ACT : CLR_BTN_BG), LV_PART_MAIN);
    ...
    static void onSleepBtn(lv_event_t* e) {
        ...
        if (idx >= SLEEP_OPTION_COUNT) return;
    ...
    static void onRotBtn(lv_event_t* e) {
        uint32_t idx = reinterpret_cast<uintptr_t>(lv_event_get_user_data(e));
        if (idx >= ROT_OPTION_COUNT) return;
    ...
    lv_obj_t* makeRow(lv_obj_t* parent, int16_t y, int16_t h,
                      const char* label, lv_obj_t** valueLabel) {
    ...
    lv_obj_t* makeSegButton(lv_obj_t* parent, const char* label, bool active,
        ...
        lv_obj_set_style_border_color(btn, lv_color_hex(active ? CLR_ACCENT : CLR_BTN_BDR), LV_PART_MAIN);
...
void SettingsPage::init(int16_t yOffset, int16_t height) {
    ...
    y += gapRow;
    {
        ...
        for (uint8_t i = 0; i < SLEEP_OPTION_COUNT; ++i) {
            bool active = (SLEEP_OPTIONS[i] == s_sleepTimeoutS);
            s_sleepBtns[i] = makeSegButton(s_panel, SLEEP_LABELS[i], active,
                                           onSleepBtn, reinterpret_cast<void*>(static_cast<uintptr_t>(i)));
    ...
    y += gapRow;
    {
        ...
        for (uint8_t i = 0; i < ROT_OPTION_COUNT; ++i) {
            ...
            bool active = (ROT_OPTIONS[i] == s_rotation);
            s_rotBtns[i] = makeSegButton(s_panel, rotLabel, active,
                                         onRotBtn, reinterpret_cast<void*>(static_cast<uintptr_t>(i)));
...
void SettingsPage::applyFromUsb(uint8_t brightness, uint8_t contrastPct,
                                uint32_t sleepTimeoutS, uint16_t rotation) {
    // Validate and clamp
    if (brightness < 10 || brightness > 100) brightness = DEFAULT_BRIGHTNESS;
    if (contrastPct > 100)                   contrastPct = DEFAULT_CONTRAST;
    ...
    if (s_brSlider) lv_slider_set_value(s_brSlider, s_brightness, LV_ANIM_OFF);
    if (s_ctSlider) lv_slider_set_value(s_ctSlider, s_contrast,   LV_ANIM_OFF);
    updateBrValue();
    updateCtValue();
    updateSleepButtons();
    updateRotButtons();
    ...
    LOG_INFO("Settings", "Applied from USB — brightness=%d%% contrast=%d%%",
             s_brightness, s_contrast);
}
